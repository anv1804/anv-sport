...
export default function MatchDetailClient({ matchId }: { matchId: string }) {
  ...
  // Hook for background auto-generation based on match state
  useEffect(() => {
    if (!matchInfo || loadingMatch || !historyLoaded || autoGenerated || isGenerating) return;
    ...
    const hasLineups = matchInfo.lineups && matchInfo.lineups.length === 2 && matchInfo.lineups[0].startXI?.length > 0;

    const triggerAutoGeneration = async () => {
      setAutoGenerated(true);

      // Case 1: Pre-match, lineups are announced, and there's no pre-match lineup prediction saved yet.
      // Automatically generate once and SAVE it in database permanently.
      if (!isLive && hasLineups) {
        const hasLineupPreMatch = predictionHistory.some(hist => 
          hist.milestone === 'PRE_MATCH' && 
          (hist.status.toLowerCase().includes('lineup') || hist.status === 'Đã công bố đội hình')
        );
        if (!hasLineupPreMatch) {
          setLoadingText("Đã công bố đội hình! Siêu máy tính đang tự động tạo nhận định trước trận...");
          await handleGenerateInternal(false);
          return;
        }
      }

      // Case 2: LIVE match - auto generate real-time preview (previewOnly: true) to avoid spamming DB
      // Case 3: No predictions at all yet - auto generate a real-time preview
      if (isLive || predictionHistory.length === 0) {
  ...
  }, [matchInfo, predictionHistory, loadingMatch, historyLoaded, autoGenerated, isGenerating]);
  ...
  const handleGenerateInternal = async (previewOnly: boolean = false) => {
    if (!matchInfo) return;
    ...
    try {
      const res = await fetch('/api/generate-prediction', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title: query, matchData: matchInfo, previewOnly })
      });
      const data = await res.json();
        ...
        {!isFinished && (
          <div className="p-8 md:p-12 bg-white text-center">
            ...
            <p className="text-slate-500 text-[13px] font-medium mb-6 max-w-md mx-auto">
              Hệ thống tự động phân tích và đưa ra nhận định chuyên sâu theo thời gian thực. Bạn có thể chủ động lưu trữ các mốc dự báo này vào dòng thời gian lịch sử nếu cần thiết.
            </p>
                ...
                {isDataPreview ? (
                  <>
                    <Button 
                      ...
                      className="bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded text-[13px] font-black uppercase tracking-widest shadow-md flex items-center justify-center transition-colors"
                    >
                      <Bot className="w-4 h-4 mr-2" />
                      Lưu mốc phân tích này vào lịch sử
                    </Button>
                    {isLive && (
                      <Button 
                      ...
                      >
                        Cập nhật trực tiếp mới nhất (Không lưu)
                      </Button>
                    )}
                  </>
                ) : (
