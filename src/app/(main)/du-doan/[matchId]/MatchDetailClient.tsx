...
export default function MatchDetailClient({ matchId }: { matchId: string }) {
  ...
  useEffect(() => {
    ...
    const triggerAutoGeneration = async () => {
      ...
      // Milestone 1: Trước trận (PRE_MATCH) - Trước khi trận đấu diễn ra
      if (!isLive) {
        const hasPreMatch = predictionHistory.some(hist => hist.milestone === 'PRE_MATCH');
        if (!hasPreMatch) {
          setLoadingText("Siêu máy tính đang tạo nhận định trước trận đấu...");
          await handleGenerateInternal(false, 'PRE_MATCH');
          return;
        }
      }

      // Milestone 2: Đầu trận (START_MATCH) - Khi trọng tài bắt đầu thổi còi bắt đầu hiệp 1
      if (isLive) {
        ...
        if (isStartPeriod) {
          ...
          if (!hasStartMatch) {
            setLoadingText("Trận đấu bắt đầu hiệp 1! Siêu máy tính đang tạo nhận định đầu trận...");
      ...
      // Milestone 3: Giữa trận (HALF_TIME) - Khi trọng tài thổi còi bắt đầu hiệp 2
      if (isLive) {
        const livePeriodLower = (matchInfo.livePeriod || '').toLowerCase();
        const isSecondHalfStart = statusLower.includes('2nd half') || statusLower.includes('hiệp 2') || livePeriodLower.includes('2nd') || livePeriodLower.includes('hiệp 2');
        if (isSecondHalfStart) {
          const hasHalfTime = predictionHistory.some(hist => hist.milestone === 'HALF_TIME');
          if (!hasHalfTime) {
            setLoadingText("Trọng tài bắt đầu hiệp 2! Siêu máy tính đang tạo nhận định giữa trận...");
