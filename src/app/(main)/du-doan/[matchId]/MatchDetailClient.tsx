...
export default function MatchDetailClient({ matchId }: { matchId: string }) {
  ...
  useEffect(() => {
    ...
    const triggerAutoGeneration = async () => {
      ...
      // Milestone 1: PRE_MATCH (Trước trận đấu hoàn toàn)
      if (!isLive) {
        const hasPreMatch = predictionHistory.some(hist => hist.milestone === 'PRE_MATCH');
        if (!hasPreMatch) {
          setLoadingText("Siêu máy tính đang tự động tạo nhận định trước trận đấu...");
          await handleGenerateInternal(false, 'PRE_MATCH');
          return;
        }
      }

      // Milestone 2: START_MATCH (Bắt đầu trận)
      if (isLive) {
        const liveClockStr = matchInfo.liveClock || '';
        const elapsedMin = parseInt(liveClockStr.replace("'", "")) || 0;
        const isStartPeriod = !statusLower.includes('ht') && !statusLower.includes('giữa hiệp') && elapsedMin <= 5;
        if (isStartPeriod) {
          const hasStartMatch = predictionHistory.some(hist => hist.milestone === 'START_MATCH');
          if (!hasStartMatch) {
            setLoadingText("Trận đấu bắt đầu! Siêu máy tính đang tạo nhận định khai cuộc...");
      ...
      // Milestone 3: HALF_TIME (Giữa trận)
      const isHT = statusLower.includes('ht') || statusLower.includes('giữa hiệp') || matchInfo.livePeriod === 'HT';
      if (isHT) {
        const hasHalfTime = predictionHistory.some(hist => hist.milestone === 'HALF_TIME');
        if (!hasHalfTime) {
          setLoadingText("Nghỉ giữa hiệp! Siêu máy tính đang phân tích chiến thuật hiệp 2...");
          await handleGenerateInternal(false, 'HALF_TIME');
          return;
        }
      }

      // Fallback: If no predictions at all yet - auto generate a real-time preview (not saved)
